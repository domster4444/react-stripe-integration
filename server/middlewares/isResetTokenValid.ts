...
var checkIfUserIsLoggedIn = async (req: Request, res: Response, next: any) => {
  ...
  if (authorization && authorization.startsWith('Bearer')) {
    try {
       ...
       */
      const { _id } = jwt.verify(token, process.env.JWT_RESET_PASSWORD); // even if it gets verified , we still need to check if userId of token we got is valid or not(exist in db or not)
      ...
      //@ts-ignore
      req.user = await userModel.findById(_id).select('-password'); // provided all data of user except password
      next();
    } catch (err) {
      console.log(err);
      res.status(401).json({
